...
class StandardTrainerLoader(TrainerLoader):
    ...
    def translate_config(self) -> TrainingArguments:
        ...
        # Build kwargs for TrainingArguments; omit save_safetensors if this
        # transformers version does not support it (avoids Colab/version skew).
        ...
        kwargs: dict[str, Any] = {
            ...
            "optim": getattr(
                self.calt_config, "optimizer", _DEFAULT_TRAINING_ARGS.optim
            ),
        ...
        if "save_safetensors" in supported:
            kwargs["save_safetensors"] = getattr(
                self.calt_config, "save_safetensors", False
            )
